...
export class KKPParser implements BankParser {
  id = 'kkp';
  name = 'KKP Mobile';
  ...
  detect(text: string): boolean {
    ...
    return (
      textLower.includes('kkp') ||
      textLower.includes('kiatnakin') ||
      textLower.includes('เกียรตินาคิน')
    );
  }

  parse(text: string): ParsedTransaction {
    const result = createBaseResult(text, 'kkp');
    ...
    result.counterparty = counterparty || (result.type === 'income' ? 'KKP Inflow' : 'KKP Outflow');
